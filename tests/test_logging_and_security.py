...
def test_dbir_rag_tool_logs_error_on_exception(mocker, caplog):
    ...
    mocker.patch("src.tools.retriever.query_dbir_report", return_value="Se produjo un error al intentar recuperar información del informe DBIR.")
    
    # Mockear OpenAIEmbeddings().embed_query para que devuelva una lista de floats
    mock_embeddings_instance = MagicMock()
    mock_embeddings_instance.embed_query.return_value = [0.1, 0.2, 0.3] # Simular un embedding válido
    mocker.patch("src.tools.retriever.OpenAIEmbeddings", return_value=mock_embeddings_instance)
    ...
    assert "Se produjo un error al intentar recuperar información del informe DBIR." in result
